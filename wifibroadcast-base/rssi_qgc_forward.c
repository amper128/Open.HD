...
wifibroadcast_rx_status_t *status_memory_open() {
    ...
    if (fd < 0) {
        fprintf(stderr, "ERROR: Could not open wifibroadcast_rx_status_0");
...
wifibroadcast_rx_status_t *status_memory_open_tdown() {
    ...
    if (fd < 0) {
        fprintf(stderr, "ERROR: Could not open wifibroadcast_rx_status_1");
...
wifibroadcast_rx_status_t_sysair *status_memory_open_sysair() {
    int fd = shm_open("/wifibroadcast_rx_status_sysair", O_RDWR, S_IRUSR | S_IWUSR);
...
wifibroadcast_rx_status_t_rc *status_memory_open_rc() {
    int fd = shm_open("/wifibroadcast_rx_status_rc", O_RDWR, S_IRUSR | S_IWUSR);
   
    if (fd < 0) {
        fprintf(stderr, "ERROR: Could not open wifibroadcast_rx_status_rc");
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        if (sendto(s_rssi, &wbcdata, sizeof(wifibroadcast_rx_status_forward_t), 0, (struct sockaddr *)&si_other_rssi, slen_rssi) == -1) {
