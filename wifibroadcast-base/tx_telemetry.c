...
uint8_t headers_atheros[40];

// header buffer for ralink
uint8_t headers_ralink[40];

// header buffer for Realtek
uint8_t headers_Realtek[40];
...
uint8_t packet_buffer_ath[402]; 
uint8_t packet_buffer_ral[402];
uint8_t packet_buffer_rea[402];


// telemetry frame header consisting of seqnr and payload length
 // TODO: check if packed works as intended
struct header_s {
    uint32_t seqnumber;
    uint16_t length;
    ...
    if (transmission_mode == 1) {
        ...
        switch (type[best_adapter]) {
            case 0: {
                ...
                if (write(socks[best_adapter], &packet_buffer_ral, headers_ralink_len + 6 + len + padlen) < 0) {
                    fprintf(stderr, ".");
                    exit(1);
                }
            ...
            case 1: {
                ...
                if (write(socks[best_adapter], &packet_buffer_ath, headers_atheros_len + 6 + len + padlen) < 0) {
                    fprintf(stderr, ".");
                    exit(1);
                }
            ...
            case 2: {
                ...
                if (write(socks[best_adapter], &packet_buffer_rea, headers_Realtek_len + 6 + len + padlen) < 0) {
                    fprintf(stderr, ".");
                    exit(1);
                }
        ...
        for (i = 0; i < num_int; ++i) {
            switch (type[i]) {
                case 0: {
                    ...
                    if (write(socks[i], &packet_buffer_ral, headers_ralink_len + 6 + len + padlen) < 0) {
                        fprintf(stderr, ".");
                        exit(1);
                    }
                ...
                case 2: {
                    ...
                    if (write(socks[i], &packet_buffer_rea, headers_Realtek_len + 6 + len + padlen) < 0) {
                        fprintf(stderr, ".");
                        exit(1);
                    }
...
int main(int argc, char *argv[]) {
    ...
    if (param_cts == 1) {
        ...
        // radiotap header
        memcpy(headers_atheros, u8aRadiotapHeader, sizeof(u8aRadiotapHeader));
        // ieee header
        memcpy(headers_atheros + sizeof(u8aRadiotapHeader), u8aIeeeHeader_data, sizeof(u8aIeeeHeader_data));
        ...
        memcpy(headers_atheros, u8aRadiotapHeader, sizeof(u8aRadiotapHeader));
        // ieee header
        memcpy(headers_atheros + sizeof(u8aRadiotapHeader), u8aIeeeHeader_rts, sizeof(u8aIeeeHeader_rts));
